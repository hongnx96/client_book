<template>
    <div class="row mt-2">
        ...
        <div class="col-4">
            <h5>Update author</h5>
            <form @submit.prevent="updateAuthorById">
    ...
    methods: {
        ...
        getAuthorById() {
            ...
            .then((response) => {
                if(response.data.author) {
                    ...
                    let date = new Date(response.data.author[0].author_day_of_birth);
                    let day = ("0" + date.getDate()).slice(-2);
                    let month = ("0" + (date.getMonth() + 1)).slice(-2);
                    let year = date.getFullYear();
                    this.inputName = response.data.author[0].author_name;
                    this.inputDayBirth = [day, month, year].join("-");
        ...
        updateAuthorById() {
            ...
            if(this.$v.$invalid) {
                ...
                this.$store.dispatch('AUTHOR/updateAuthorById', {
                    id: this.$route.params.id,
                    // id: this.inputId,
                ...
                .then((response) => {
                    if(response.data.success) {
                        this.inputName = '';
                        this.inputDayBirth = '';
                        this.msgUpdate = 'Update author successfull.';
                        this.$router.push({
                            name: 'AuthorIndex'
                        });
